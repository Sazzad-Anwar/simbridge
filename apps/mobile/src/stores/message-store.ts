...
interface MessageState {
  ...
  /** Clear a previous decrypt failure so the entry can be retried. */
  clearDecryptError: (messageId: string) => void
...
export const useMessageStore = create<MessageState>((set, get) => ({
  ...
  async drainNativeOutbox() {
    return serializeRecovery(async () => {
      ...
      const active = pairs.find(
        (p) => p.status === 'active' && p.receiverPublicKey,
      )
  ...
  async reconcileInbox() {
    return serializeRecovery(async () => {
      ...
      let maxTs = after
      for (const raw of native) {
        if (raw.timestamp != null && raw.timestamp > maxTs)
          maxTs = raw.timestamp
        const body = raw.body?.trim()
        const sender = raw.originatingAddress?.trim()
        if (!body || !sender) continue
      ...
      if (candidates.length > 0) {
        try {
          ...
          console.log(
            '[smsbridge] reconcileInbox: exists pre-check failed',
            String(err),
          )
        }
      }
      ...
      console.log(
        '[smsbridge] reconcileInbox: forwarded',
        forwarded,
        'of',
        native.length,
      )
      ...
      try {
        ...
        if (role === 'sender' && fetched.length > 0) {
          ...
          const statusById = new Map(
            fetched.map((m) => [m.messageId, m.status]),
          )
          const outbox = await storage.getOutbox()
          let dirty = false
          for (const entry of outbox) {
            const remote = entry.messageId
              ? statusById.get(entry.messageId)
              : undefined
  ...
  async decryptEntry(entry) {
    if (entry.decrypted !== undefined) return entry.decrypted
    const secret = await receiverSecret()
    let decrypted: string | undefined
    let decryptError: InboxEntry["decryptError"]
    if (!secret) {
      decryptError = "no-key"
    } else {
      try {
    ...
    const inbox = get().inbox.map((m) =>
      m.messageId === entry.messageId
        ? { ...m, ...(decrypted !== undefined ? { decrypted } : { decryptError }) }
        : m,
    )
    set({ inbox })
  ...
  clearDecryptError(messageId) {
    const inbox = get().inbox.map((m) =>
      m.messageId === messageId ? { ...m, decryptError: undefined } : m,
    )
    set({ inbox })
  },
