...
export default function RoleSelect() {
  ...
  return (
    <Screen style={{ justifyContent: "center", gap: 16 }}>
      <Title style={{ fontSize: 24, textAlign: "center" }}>Choose your role</Title>
      ...
      <Pressable onPress={() => void choose("sender")} disabled={busy !== null}>
        {({ pressed }) => (
          <Card style={[styles.roleCard, pressed && { opacity: 0.8 }, busy === "receiver" && { opacity: 0.4 }]}>
            <Row>
            ...
            <Muted style={{ fontSize: 11 }}>Needs SMS permissions + a persistent foreground service</Muted>
          </Card>
        )}
      </Pressable>

      <Pressable onPress={() => void choose("receiver")} disabled={busy !== null}>
        {({ pressed }) => (
          <Card style={[styles.roleCard, pressed && { opacity: 0.8 }, busy === "sender" && { opacity: 0.4 }]}>
            <Row>
              <Text style={{ fontSize: 32 }}>🔔</Text>
              <View style={{ flex: 1 }}>
      ...
      {error ? <Muted style={{ color: colors.red, textAlign: "center" }}>{error}</Muted> : null}
