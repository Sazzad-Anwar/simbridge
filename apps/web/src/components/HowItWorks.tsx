    ...
    >
      ...
      <StaggerGroup className="mx-auto mt-14 grid max-w-6xl gap-6 px-4 sm:px-6 md:grid-cols-3">
        {STEPS.map((step) => {
          ...
          return (
            ...
            >
              <span
                className="absolute right-6 top-5 text-5xl font-extrabold text-line/80 transition-colors group-hover:text-blue/15"
                aria-hidden
              >
                {step.n}
              </span>
              <span
                className={`grid h-14 w-14 place-items-center rounded-2xl bg-linear-to-br ${step.accent} text-white shadow-md`}
              >
                <Icon
                  className="h-7 w-7"
                  aria-hidden
      ...
      <SectionReveal className="mx-auto mt-12 max-w-6xl px-4 sm:px-6">
        <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-3 rounded-2xl border border-blue/20 bg-bg-blue/70 px-6 py-4 text-sm font-medium text-muted">
          ...
          <span
            className="hidden h-1 w-1 rounded-full bg-blue/40 sm:block"
            aria-hidden
          />
          <span className="flex items-center gap-2">
            <Link2
