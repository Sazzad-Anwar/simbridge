...
import { isDeviceOnline, getIo } from "../realtime/io.js";
...
export async function acceptPair(
  ...
  if (!updated) throw errors.pairCodeExpired();

  const sender = await Device.findOne({ deviceId: updated.senderDeviceId }).lean<DeviceDoc>();

  // Put every connected socket of both devices into the pair room.
  const io = getIo();
  io.in(deviceRoom(updated.senderDeviceId)).socketsJoin(pairRoom(roomId));
  io.in(deviceRoom(receiver.deviceId)).socketsJoin(pairRoom(roomId));
  io.to(deviceRoom(updated.senderDeviceId)).emit(SocketEvents.PAIRING_ACCEPTED, {
  ...
  audit("pair.accepted", { deviceId: receiver.deviceId, pairId: updated.pairId });
  return {
    pairId: updated.pairId,
    roomId,
